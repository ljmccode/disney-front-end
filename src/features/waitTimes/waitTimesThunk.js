...
export const getParkTimesThunk = async (_, thunkAPI) => {
  ...
  let url = `https://disney-wait-times.herokuapp.com/api/v1/${currentPark}?date=${date}`;
  try {
...
export const getCurrentWaitTimesThunk = async (_, thunkAPI) => {
  ...
  let url = `https://disney-wait-times.herokuapp.com/api/v1/currentWait/${currentPark}`;
  try {
    const { data } = await axios(url);
    return data;
  } catch (error) {
